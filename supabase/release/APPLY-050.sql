-- ============================================================
-- RELEASE — 050_issues, satu tampalan
--
-- LANGKAH PERTAMA, SEBELUM APA-APA: jalankan query semakan di bawah
-- SEORANG DIRI. Jangan tampal fail ini sebelum ia mengembalikan SIFAR
-- baris.
--
--   SELECT account_id, lower(code), count(*)
--     FROM centres
--    WHERE code IS NOT NULL AND code <> ''
--    GROUP BY 1, 2
--   HAVING count(*) > 1;
--
-- Kenapa langkah berasingan, dan bukan sekadar nasihat:
--
--   Migration ini mencipta indeks UNIK pada kod cawangan. Kalau pendua
--   sudah wujud, CREATE UNIQUE INDEX gagal — dan kerana semuanya dalam
--   SATU transaksi, kegagalan itu menggulung balik jadual issues dan
--   issue_events sekali. Tiada yang mendarat.
--
--   Itu bukan kerosakan; itu rangkaian keselamatan berfungsi. Tetapi ia
--   bermakna lima minit menjalankan query di atas menjimatkan satu
--   larian yang gagal sepenuhnya.
--
-- KALAU QUERY ITU MENGEMBALIKAN BARIS
--   Jangan teruskan. Setiap baris ialah dua cawangan atau lebih yang
--   berkongsi satu kata kunci — maksudnya mesej ibu bapa sedang menanda
--   cawangan yang SALAH hari ini. Betulkan kodnya dahulu dalam
--   Tetapan → Pusat, kemudian jalankan query sekali lagi.
--
-- PERHATIAN: predikat dalam query itu (`code <> ''`) mesti sepadan
-- dengan predikat indeks. Rentetan kosong bermaksud "tiada kod", sama
-- seperti NULL, dan kedua-duanya berada di LUAR indeks. Query semakan
-- yang tidak sepadan akan melaporkan bersih sementara indeks tetap
-- gagal.
--
-- CARA GUNA
--   Supabase Dashboard → SQL Editor → New query → tampal SEMUA → Run.
--
-- Ia SATU TRANSAKSI. Semuanya mendarat atau tiada satu pun.
--
-- DISEMAK SEBELUM DIGABUNG
--   Tiada CONCURRENTLY, VACUUM, ALTER SYSTEM atau ADD VALUE — semuanya
--   selamat dalam transaksi.
--
--   050 tidak menyentuh apa-apa yang 042, 043, 044 atau 048 cipta.
--   Keempat-empatnya ada dalam repo dan TIADA pada production, jadi
--   sebarang kebergantungan padanya akan lulus CI (yang memainkan
--   semula semuanya) dan pecah di sini. Disahkan dengan menjalankan 050
--   terhadap baseline tanpa keempat-empatnya.
--
--   is_account_member() yang dipanggil oleh polisi di bawah ialah versi
--   017 yang hidup pada production, bukan tulisan semula 044.
--
-- SELEPAS RUN
--   Query terakhir mencetak enam baris semakan. Kesemuanya mesti 't'.
--   Kalau mana-mana 'f', beritahu dan JANGAN sambung nombor.
-- ============================================================
...
-- ============================================================
-- SEMAKAN — kesemua enam mesti 't'
-- ============================================================
SELECT
  to_regclass('public.issues')        IS NOT NULL AS jadual_issues,
  to_regclass('public.issue_events')  IS NOT NULL AS jadual_issue_events,
  (SELECT relrowsecurity FROM pg_class
    WHERE oid = 'public.issues'::regclass)        AS rls_issues,
  (SELECT relrowsecurity FROM pg_class
    WHERE oid = 'public.issue_events'::regclass)  AS rls_issue_events,
  to_regclass('public.idx_centres_account_code') IS NOT NULL AS indeks_kod_unik,
  -- Ketiadaan polisi UPDATE pada issue_events ialah reka bentuk, bukan
  -- terlupa: RLS menolak apa yang ia tidak benarkan, jadi tiada
  -- pernyataan yang boleh menyunting jejak audit. Disemak di sini
  -- kerana sesuatu yang TIADA tidak akan disedari bila ia muncul.
  NOT EXISTS (
    SELECT 1 FROM pg_policy
     WHERE polrelid = 'public.issue_events'::regclass AND polcmd = 'w'
  ) AS jejak_audit_tak_boleh_disunting;
